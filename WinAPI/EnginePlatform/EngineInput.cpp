...
// UEngineInput 내부의 UengineKey 클래스
void UEngineInput::UEngineKey::KeyCheck(float _DeltaTime)
{
	// GetAsynKeyState(KeyCode) : 어떤 키가 입력되었는지 여부와 이전 호출 여부를 확인
	if (0 != GetAsyncKeyState(Key))	// 해당 키 입력이 있었다면
	{
		PressTime += _DeltaTime;

		if (true == IsFree)			// 이전까지 미입력 상태였다면
		{
			IsDown = true;
			IsPress = true;
			IsFree = false;
		...
		else if (true == IsDown)	// 이미 키가 눌려있었을 경우
		{
			IsDown = false;
			IsPress = true;
			IsFree = false;
	...
	{
		PressTime = 0.0f;			// 키 입력 시간 초기화

		if (true == IsPress)		// 키가 눌려있었던 상태라면
		{
			IsDown = false;
			IsPress = false;
			IsFree = true;
...
{
	// 해당 키 상태가 true라면 키 이벤트 벡터에 쌓여있는 이벤트 함수 차례로 호출
	if (true == IsDown)
	{
...
bool UEngineInput::IsDown(int _KeyIndex)
{
	// contains : <set> 또는 <map>에서 특정 키 존재 여부 확인
	// C++ 20부터 사용 가능
...
bool UEngineInput::IsPress(int _KeyIndex)
{
	if (false == Keys.contains(_KeyIndex))
	{
		MSGASSERT("아직도 등록되지 않은 키가 존재합니다.");
...
bool UEngineInput::IsUp(int _KeyIndex)
{
	if (false == Keys.contains(_KeyIndex))
	{
		MSGASSERT("아직도 등록되지 않은 키가 존재합니다.");
...
bool UEngineInput::IsFree(int _KeyIndex)
{
	if (false == Keys.contains(_KeyIndex))
	{
		MSGASSERT("아직도 등록되지 않은 키가 존재합니다.");
...
float UEngineInput::IsPressTime(int _KeyIndex)
{
	if (false == Keys.contains(_KeyIndex))
	{
		MSGASSERT("아직도 등록되지 않은 키가 존재합니다.");
...
void UEngineInput::EventCheck(float _DletaTime)
{
	// 해당 UEngineKey의 EventCheck 호출
	std::map<int, UEngineKey>::iterator StartIter = Keys.begin();
	std::map<int, UEngineKey>::iterator EndIter = Keys.end();
...
{
	// 키 입력에 따른 이벤트 함수 벡터에 해당 이벤트 함수 추가(연결)
	// 이벤트 발생 시 UEngineKey의 EventCheck로 해당 함수 호출하게끔 함.
	if (false == Keys.contains(_KeyIndex))
	{
		MSGASSERT("아직도 등록되지 않은 키가 존재합니다.");
		return;
